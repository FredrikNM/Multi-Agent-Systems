...
task_worktime = 1		# Time it takes for completing a task. If you want to scale it, you can say linear vs agents = True
task_worktime_linear_vs_agents = True		# Meaning if work time is divided by agents working on the task
agents_needed_for_a_task = 2	# How many agents that is need for a task to be completed
agents_max_waiting_time = 0	# How long will an agent wait to get help for a task found, if 0 they will wait infinitly long
signal_radius = 600		# Radius of signal sent out when a task is found
signal_search_time = 0		# Time steps agents follow signals
signal_call_off = False
...
# Radius is the size of task radius (Will look very good with discretization around 20, and then you can save instead and watch it)
...
# Will simulate for call off signal = True, and call off = false with signal_search_time = 60, and auction 
n_different_agents = [30]
n_different_signal_radius = [0, 100, 200, 300, 400, 600, 1000, 1400]
n_different_signal_radius = [1000, 1400]
...
def simulate_movement(agents, agent_radius, x_y_walls, steps_per_timeunit, movement_function, \
	...
	if bidding_agents == True:
		global sorted_bids, work_and_signal_matrix
	...
	# Live signals
	live_signal = np.where(signal_matrix == 1)[0]


	# Checking for signals
	if len(live_signal) > 0:

		# Finding distances for each agent to signals from working agents
		signaldist = cdist(agents_position, agents_position[np.unique(live_signal)])
		...
		agent_in_signal_dist = np.setdiff1d(agent_in_signal_dist, np.unique(live_signal))
		# Resetting bids put
		if bidding_agents == True:
			for n in range(len(agents)):
				agents["A"+str(n)].all_bids = {}

		if len(agent_in_signal_dist) > 0:

			# Saving agents that have recived a signal, and from whom
			agent_sending_signal = np.unique(live_signal)
			for n in range(len(agent_sending_signal)):
				if bidding_agents == True:
					agents_recived = np.array(list(set(np.where(signaldist[:,n] < signal_radius)[0]).intersection(set(agent_in_signal_dist))))
				else:
					agents_recived = np.array(list(set(np.where(signals_closest == n)[0]).intersection(set(agent_in_signal_dist))))

				agents_recived = np.array([a_x for a_x in agents_recived if agents["A"+str(a_x)].work != True and agents["A"+str(a_x)].signal != True])

				if len(agents_recived):

					signal_reciver[agent_sending_signal[n]] = agents_recived

					if bidding_agents == True:

						sorted_bids = agents["A"+str(agent_sending_signal[n])].auctioneer(agents_position[agents_recived])
						closest_agents = signal_reciver[agent_sending_signal[n]][sorted_bids[0]]
						signal_reciver[agent_sending_signal[n]] = closest_agents
						for k in range(len(closest_agents)):
							agents["A"+str(closest_agents[k])].bidder([np.where(work_matrix[agent_sending_signal[n],:] == 1)[0][0], sorted_bids[1][k] ])


			if bidding_agents == True:
				preference_list = {}

				for n in range(len(signal_reciver)):
					a_n = list(signal_reciver.keys())[n]	# Auctioneer agent
					preference_list[a_n] = [agents["A"+str(k)].preference_list() for k in signal_reciver[a_n] if len(agents["A"+str(k)].all_bids)]

					if len(preference_list[a_n]):
						k = 0
						counter = 0
						agents_n_work_nr = np.where(work_matrix[a_n,:] == 1)[0][0]
						agents_heading_for_task_or_working = np.sum(work_and_signal_matrix[:,agents_n_work_nr])
						# First check if auctioneer gets their top prority
						while agents_needed_for_a_task - agents_heading_for_task_or_working > 0 and k != 1 and counter < len(preference_list[a_n]):

							# Agents are just added sequentially (meaning the first auctioneer gets his preferences, then auctioneer two...)
							direction_to_work = agents["A"+str(a_n)].work_position - agents_position[signal_reciver[a_n][counter]]
							direction[signal_reciver[a_n][counter]] = direction_to_work/np.sqrt(direction_to_work[0]**2 + direction_to_work[1]**2)
							agents["A"+str(signal_reciver[a_n][counter])].recived_signal(signal_search_time*steps_per_timeunit)
							work_and_signal_matrix[signal_reciver[a_n][counter], agents_n_work_nr] = 1
							agents_heading_for_task_or_working += 1
							counter += 1

			if bidding_agents != True:
				# Storing signals that reached the agents
				temp_signal = [agents_position[np.unique(live_signal)][signals_closest[n]] for n in agent_in_signal_dist]
				# Calculating the vector from signal to agent
				direction_to_signal = temp_signal - agents_position[agent_in_signal_dist]
				# Normalizing (can cause owerflow) if so it becomes a direction for each step
	...
	for k in range(work_matrix.shape[1]):
		...
		if len(working_idx) >= agents_needed_for_a_task:
			...
			# If task duration is completed the agents are set free to roam again, and a new task is spawned
				...
				for n in working_idx:
					agents["A"+str(n)].work = False
					agents["A"+str(n)].work_position = None
					signal_reciver.pop(n, None)
				work_matrix[:,k] = 0
				if bidding_agents == True:
					work_and_signal_matrix[:,k] = 0
				signal_matrix[:,k] = 0
				task_completed += 1
		...
		# Checking agents max waiting time at work station, if work is not started
			...
			if len(working_and_max) > 0:
				...
				if bidding_agents == True:
					work_and_signal_matrix[:,k][working_and_max] = 0
				signal_matrix[:,k][working_and_max] = 0
	...
	for k in range(steps_per_timeunit):
		...
		# Calculate what would be the new position before actually moving, to avoid crashes (very inefficient!)
		...
		if len(tasks) > 0:
			...
			if bidding_agents == True:
				work_and_signal_matrix[agent_i, task_j] = 1
			for n in agent_i:
				agents["A"+str(n)].working(tasks[np.where(work_matrix[n,:] == 1)[0][0]])
...
if number_of_plots == 0:
	...
	for n in range(len(n_different_agents)):
		...
		for k in range(len(n_different_signal_radius)):

			signal_radius = n_different_signal_radius[k]

			for z in range(3):

				if z == 0:
					bidding_agents = False
					signal_search_time = 0		# Time steps agents follow signals
				...
				if bidding_agents == True:
					sorted_bids = {}
					work_and_signal_matrix = np.zeros((n_agents, task_numbers)) 	# Matrix describing if an agent is working on a specific task
				...
				if z == 0:
					eff_call_off.append(cumulative_task[-1]/simulation_time)
				if z == 1:
					eff_call_timer.append(cumulative_task[-1]/simulation_time)
				if z == 2:
					eff_bid.append(cumulative_task[-1]/simulation_time)
					eff_bid_half.append(cumulative_task[-1]/(2*simulation_time))

				# plt.plot(( (task_worktime*np.array(cumulative_task))/((np.arange(len(cumulative_task))+1)*task_numbers)), \
				# 	label=str(n_different_agents[n])+" Agents with bid = "+str(bidding_agents))
				# plt.plot(( (task_worktime*np.array(cumulative_task))/((np.arange(len(cumulative_task))+1)*task_numbers)), \
				# 	label=str(n_different_agents[n])+" Agents with bid = "+str(bidding_agents))

	plt.plot(n_different_signal_radius, eff_bid, label = "With auction")
	plt.plot(n_different_signal_radius, eff_bid_half, label = "With auction, agent priced twice as much")
	plt.plot(n_different_signal_radius, eff_call_off, label = "Without auction, Call Off Signal")
	plt.plot(n_different_signal_radius, eff_call_timer, label = "Without auction, Follow signal for 60 time steps")
