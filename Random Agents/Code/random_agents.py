...
n_agents = 25		# How many agents to implement
...
task_numbers = 3	# Task at any timesteps
task_worktime = 1		# Time it takes for completing a task. If you want to scale it, you can say linear vs agents = True
task_worktime_linear_vs_agents = False		# Meaning if work time is divided by agents working on the task
agents_needed_for_a_task = 1	# How many agents that is need for a task to be completed
agents_max_waiting_time = 0	# How long will an agent wait to get help for a task found
signal_radius = 450		# Radius of signal sent out when a task is found
...
# When saving the plot, the time it uses doesnt matter that much. Radius is the size of task radius
